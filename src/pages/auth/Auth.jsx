...
const Auth = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // login() in UserAuthContext already stores in correct storage
      // (homi_agent for agents, homi_user for users)
      // and dispatches storage event so AgentAuthContext picks it up
      // Small delay to let AgentAuthContext state update before navigating
      setTimeout(() => redirectByRole(user), 100);
    } catch (err) {
        ...
        <div className="space-y-6">
          ...
          {step === 'identifier' && (
            <>
              ...
              <button onClick={checkUserExists} disabled={!identifier || loading}
          ...
          {step === 'password' && (
            <>
              ...
              <div className="text-center mt-6">
                <Link to="/auth/forgot-password" className="text-sm text-amber-600 hover:text-amber-700 font-medium">
                  Forgot password?
                </Link>
              </div>
...
function SocialButtons({ onAlert, action }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      <button onClick={() => onAlert(`Google ${label} coming soon!`, 'info')}
        className="flex items-center justify-center py-4 bg-white rounded-full hover:bg-gray-50 transition-all" title="Continue with Google">
      ...
      <button onClick={() => onAlert(`Facebook ${label} coming soon!`, 'info')}
        className="flex items-center justify-center py-4 bg-white rounded-full hover:bg-gray-50 transition-all" title="Continue with Facebook">
        <svg className="w-6 h-6" viewBox="0 0 24 24" fill="#1877F2">
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
        </svg>
      </button>
      <button onClick={() => onAlert(`Apple ${label} coming soon!`, 'info')}
        className="flex items-center justify-center py-4 bg-white rounded-full hover:bg-gray-50 transition-all" title="Continue with Apple">
