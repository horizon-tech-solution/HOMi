...
import {
  Activity, Search, Filter, ChevronDown,
  CheckCircle, XCircle, Ban, Shield, Flag, Trash2,
  MessageSquare, UserCheck, Building2, Clock, User,
...
const LOG = [
  ...
  { id: 16, action: 'user_unblocked', actor: 'Admin', target: 'User #u_088', detail: 'Block lifted — appeal reviewed and approved', time: '2024-12-14T09:00:00', category: 'user' },
  { id: 17, action: 'report_resolved', actor: 'Admin', target: 'Report #40 — fake listing', detail: 'Listing removed, user warned via email', time: '2024-12-13T15:00:00', category: 'report' },
  { id: 18, action: 'listing_rejected', actor: 'Admin', target: 'Land listing Bepanda (#1012)', detail: 'Rejected — no land title provided', time: '2024-12-13T11:00:00', category: 'listing' },
];

const ACTION_CFG = {
  listing_approved:  { icon: CheckCircle, label: 'Listing Approved',   cls: 'text-emerald-600 bg-emerald-50' },
  listing_rejected:  { icon: XCircle,     label: 'Listing Rejected',   cls: 'text-red-500 bg-red-50' },
  listing_flagged:   { icon: AlertTriangle,label: 'Listing Flagged',   cls: 'text-orange-500 bg-orange-50' },
  listing_deleted:   { icon: Trash2,      label: 'Listing Deleted',    cls: 'text-red-600 bg-red-50' },
...
export default function ActivityLog() {
  ...
  const filtered = LOG.filter(l => {
    ...
    const searchMatch = !q || [l.target, l.detail, l.actor].some(f => f.toLowerCase().includes(q));
  ...
  return (
    <AdminNav>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8" style={{ fontFamily: "'DM Sans', sans-serif" }}>

        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900">Activity Log</h1>
            <p className="text-sm text-zinc-400 mt-0.5">Full audit trail of all admin actions on the platform.</p>
          </div>
          <button className="flex items-center gap-1.5 text-sm font-semibold text-zinc-500 border border-zinc-200 px-4 py-2 rounded-lg hover:bg-zinc-50 transition-colors">
            ...
            <div key={date}>
              ...
              <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden divide-y divide-zinc-100">
                {entries.map((log, i) => {
                  ...
                  const Icon = cfg.icon;
                  return (
                    <div key={log.id} className="flex items-start gap-4 px-5 py-4 hover:bg-zinc-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-semibold text-zinc-800">{log.target}</p>
                        <p className="text-xs text-zinc-400 mt-0.5">{log.detail}</p>
                      </div>
