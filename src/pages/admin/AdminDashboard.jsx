...
import {
  ...
  ArrowRight, Eye, ChevronRight, Activity
} from 'lucide-react';
...
const StatCard = ({ label, value, sub, trend, trendUp, icon: Icon, onClick }) => (
  <button
    onClick={onClick}
    className="group bg-white border border-zinc-200 rounded-xl p-5 text-left hover:border-zinc-300 hover:shadow-sm transition-all"
  >
    ...
    <p className="text-2xl font-bold text-zinc-900 mb-0.5">{fmt(value)}</p>
...
const PendingRow = ({ item, onApprove, onView }) => {
  ...
  return (
    <div className="flex items-center gap-4 py-3.5 border-b border-zinc-100 last:border-0">
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        <span className="text-xs text-zinc-400 mr-2 hidden sm:block">{item.age}</span>
        <button
          onClick={() => onView(item)}
          ...
          <div className="lg:col-span-3">
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
                ...
                <button onClick={() => navigate('/admin/listings')} className="text-xs text-zinc-400 hover:text-zinc-900 flex items-center gap-1 transition-colors">
